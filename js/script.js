...
function checkLogin(event){
	...
	} else if(inputUsername!==usernameForCheck || inputPassword!==passwordForCheck) {
...
function checkRegister(event){
	event.preventDefault(); // sa prim default action de pe form sa trimita pe pagina newsletter
...
$.get("https://jsonplaceholder.typicode.com/users", function(result) {
    ...
    for (let i = 0; i < result.length; i++) {
		...
		`); 
      	...
      	$("#dropdown-friends-list").append(`<li id="error-message">Can't acess friends list.</li>`);
    });
        ...
        let imgCount = i +1;
 		...
 		$("main").append(`
           <article>
                ...
                <div class="content">
                    <img src="http://www.facetheforce.today/random/400?r=${imgCount}">
                    <a href="#">
                    	<h4>${title}</h4>
                    </a>
                    <p>${body}</p>
...
let addsSliderImages = [
							...
							{imgSource: "img/adds/add-img-2.jpg", addText: "Gandy Digital - Best digital services"},
							{imgSource: "img/adds/add-img-3.jpg", addText: "Find the latest deals on Amazon"}
						];
let x = 0;
let timer;
...
function next() {
  	...
  	document.getElementById("addsImage").setAttribute("src", addsSliderImages[x].imgSource);
  	document.getElementById("addsText").setAttribute("src", addsSliderImages[x].imgSource);
